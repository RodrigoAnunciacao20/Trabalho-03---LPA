...
float DetM(struct matriz *M, int i, int j){  //Função que calcula o determinante de uma matriz nxn
	if(i<M->linha&&j<M->coluna){             //int i e int j são números auxiliares na orgaização da função, eles representam a posição de inicio
		float Det = 0;
		...
		return (1);  //retorna 1 quando a função já percorreu toda a matriz, fechando o ciclo
	}
}
